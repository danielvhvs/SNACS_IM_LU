...
def save_runs_2(algorithms, G, k_max, p, mc, path, n_workers):
    ...
    for name, algo in algorithms.items():
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
            futures = [
                executor.submit(single_run, G, i, p, mc, path, name, algo)
...
def main():
    ...
    parser.add_argument(
        "-r",
        "--running",
        help="whether to do runs or plot [run,spread,time,both]",
        default="run",
        type=str,
    )
    parser.add_argument(
        "-k", "--kvalue", help="the seed size to find", default=None, type=int
    )
    parser.add_argument(
        "-p", "--plot_data", help="the data to plot", default="wiki", type=str
    )
    parser.add_argument(
        "-n", "--n_workers", help="number of workers", default="1", type=int
    )
